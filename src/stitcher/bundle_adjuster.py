...
class BundleAdjuster:
  ...
  def run(self):
    ...
    while (itr_count < MAX_ITR):
      ...
      J, JtJ = self._calculate_jacobian(best_state)#FUNCION PERRONA
      ...
      #next_error_val = pso.PSO((math.sqrt(np.mean(next_residuals**2))))
      pso_parameters = {
        'CostFunction': math.sqrt(np.mean(next_residuals**2)),
        'VarMin': -10,
        'VarMax': 10,
        'nVar': 1, 
      }
      next_error_val = pso.PSO(pso_parameters,MAX_ITR)
